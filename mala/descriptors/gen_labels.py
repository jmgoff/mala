...
def from_tabulated_q(k_mu_n,l,allowed_mus = [0],allowed_k1s=[0],tabulated_all=None):
    ks,mu,n = k_mu_n
    rank = len(l)
    ...
    for mun_tup in mun_tupped:
        ...
        for label in these_labels:
            ...
            mulab = tuple(mulab)
            nlab = tuple(nlab)
            nu = ','.join(vecstrlst) % klab + ','.join(vecstrlst) % mulab + ',' + ','.join(vecstrlst) % nlab + ',' + ','.join(vecstrlst) % lvec + '_' + Lstr_std
            #print (nu)
...
def nonangind_combined(k_mu_n):
    ...
    tupped = [ tuple([(ni,mui,ki) for ki,mui,ni in zip(*cmb)]) for cmb in combos]
...
def get_k_mu_nu_rank(nu_in):
    assert len(nu_in.split('_')) <= 4, "make sure your descriptor label is in proper format: mu0k0_mu1,mu2,mu3,k1,k2,k3,n1,n2,n3,l1,l2,l3_L1"
    nu = nu_in.split('_')[1]
    nu_splt = nu.split(',')
...
def tree_l_inters(l,L_R=0,M_R=0):
    ...
    if rank == 1:
        ...
        next_node_inters = [(L5,L6) for L5,L6 in zip(next_node_inters_l, next_node_inters_r)]
    ...
    if rank == 1:
        ...
        if rank == 2:
            ...
            for ltup in all_ls:
                ...
                for inters_i in inters:
                    li_flags = [check_triangle(node[0],node[1],inter) for node,inter in zip(by_node,inters_i)]
                    inter_flags = [check_triangle(inters_i[0],inters_i[1],inters_i[3]), check_triangle(inters_i[2],ltup[remainder], inters_i[4] ), check_triangle(inters_i[3],inters_i[4],L_R) ]
                    flags = li_flags + inter_flags
                    if inv_parity:
                        parity_all = np.sum(ltup) % 2 == 0
                    elif not inv_parity:
            ...
            for ltup in all_ls:
                ...
                for inters_i in inters:
                    li_flags = [check_triangle(node[0],node[1],inter) for node,inter in zip(by_node,inters_i)]
                    inter_flags = [check_triangle(inters_i[0],inters_i[1],inters_i[4]), check_triangle(inters_i[2], inters_i[3], inters_i[5] ), check_triangle(inters_i[4],inters_i[5],L_R) ]
                    flags = li_flags + inter_flags
                    if inv_parity:
                        parity_all = np.sum(ltup) % 2 == 0
                    elif not inv_parity:
...
def charge_labels_raw(rank,nmax,lmax,k1max,k2max,mumax=1,k1min=0, k2min=0,lmin=0,L_R=0,M_R=0,all_perms=False):
    ...
    if rank >= 4:
        print ('WARNING! THIS FUNCTION ALONE DOES NOT ENSURE LINEAR DEPENDENCE BEYOND RANK 3 - NEED TO APPLY PA RULES AS WITH CHARGE INDEPENDENT BASES STILL')
    ...
    for k1 in k1rng:
        for mu0 in murng:
            for cmbo in itertools.product(k2s,mus,ns,ls):
                ...
                x = [(ksplt[i],musplt[i],lsplt[i],nsplt[i]) for i in range(rank)]
                srt = sorted(x)
                if not all_perms:
                    conds = x==srt
                elif all_perms:
                ...
                if conds:
                    ...
                    stmp = '%d-%d_' % (mu0,k1) +  munlstr  % tuple(musplt+ksplt+nsplt+lsplt)
                    #if stmp not in munl:
...
def srt_charge_basis(nulst, mu0s = [0]):
    ...
    #q_indep = {}
    #q_dep_sparsea = {} # degenerate only (no neighbor only)
    #q_dep_sparseb = {} # degenerate only (with neighbor only)
    #q_dep_complete = {} # all charge
    used_nus =[]
    for nu in nulst:
        ...
        cond2 = ki == 0 and kj == 999 # no onsite charge, non-degenerate neighbor charge
        cond3 = ki == 0 and kj != 999 and kj != 0 # no onsite charge, degenerate neighbor charge
        cond4 = ki != 0 and ki == kj # degenerate on-site and neighbor charge
        cond5 = ki != 0 and kj == 0 # onsite charge, no neighbor charge
        cond6 = ki != 0 and kj != 999 and kj != 0 and ki != kj # onsite charge, degenerate neighbor charge
        cond7 = ki != 0 and kj == 999 # nondegenerate on-site and neighbor charge
    ...
    q_dep_complete = combine_label_dcts([by_ktyp[4],by_ktyp[3],by_ktyp[2],by_ktyp[1],by_ktyp[5],by_ktyp[6]])
