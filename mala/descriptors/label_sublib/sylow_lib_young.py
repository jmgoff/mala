...
from label_sublib.sylow_lib import *
import math
...
def old_parity_filter(lnodes,linters, L_R=0):
...
class Young_Subgroup:
	...
	def degen_partitions(self,input_inds=None):
		#sigma_c_parts = self.sigma_c_partitions(semistandard=False)
		if input_inds == None:
			us = [ u for u in list(set(self.inds))]
		else:
			us = [ u for u in list(set(input_inds))]
		nuniques = len(us)
		sigma_c_parts = self.sigma_c_partitions(max_orbit=nuniques)
		return sigma_c_parts
