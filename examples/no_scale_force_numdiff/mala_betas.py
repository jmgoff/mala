...
def run_prediction(backprop=False,atoms=None,pass_descriptors=None):
    ...
    if type(atoms) == str:
        ...
        atoms = atoms
    ldos = predictor.predict_for_atoms(atoms, save_grads=backprop, pass_descriptors=pass_descriptors)
    ldos_calculator: mala.LDOS = predictor.target_calculator
    ldos_calculator.read_from_array(ldos)
    ldos_calculator.read_additional_calculation_data(os.path.join(data_path,"Be_snapshot0.out"))
...
def pre_force_callback(lmp):
    ...
    pass_descriptors = (ace_descriptors_np, local_size, fingerprint_length, True)
    #pass_descriptors = (ace_descriptors_np, local_size, fingerprint_length, False)
    print('ace descs',ace_descriptors_np[1][1][1])
    #print('ace descs',ace_descriptors_np)
    print ('positions',atoms.positions)
    ldos, ldos_calculator, parameters, predictor = run_prediction(backprop=True,atoms=atoms,pass_descriptors=pass_descriptors)
    ...
    #end energy attempt
    mala_forces = mala_forces.numpy()
    print('any nan mala forces',np.isnan(mala_forces).any())
    ...
    print ('mala_betas',mala_test[1][1][1],np.isnan(mala_test).any())
    #print("FORCE TEST: Backpropagation machinery.")
    print('mala force coeffs info:',mala_forces.shape,mala_test.shape,np.amax(mala_forces),eng)
    mala_2_lammps_step0 = mala_test.reshape((18*18*27,feature_length))
    #mala_2_lammps = mala_forces[:, 3:].flatten()
    #mala_2_lammps = mala_test.flatten()
    mala_2_lammps = mala_2_lammps_step0.flatten()
    print('dE/dB shape input for lammps',np.shape(mala_2_lammps))
    if apply_divide:
        mala_2_lammps /= mala_forces.shape[0]
    if set_zero:
        mala_2_lammps = np.zeros_like(mala_2_lammps)
    #del L
    #del lmp
    L.close()
    del mala_forces
    return np.ascontiguousarray(mala_2_lammps)
    #return np.ascontiguousarray(mala_forces.flatten())
    #return mala_forces[:, 1:].flatten()
    #return np.ascontiguousarray(mala_forces[:, 1:].flatten())
